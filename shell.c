...
int executeBasicand(char **argv)
{
    ...
    {
        execvp(argv[0],argv);
            perror(ANSI_COLOR_RED   "invalid input (la 1er commande n'est pas valide ) "   ANSI_COLOR_RESET "\n");
            return -1;
            exit(1);
...
void functionor (char** buf,int nr){
int i,pc;
    char *argv[100];
...
void functionand (char** buf,int nr){
int i,pc;
    char *argv[100];
       ...
       int ok=0;
          ...
          else if (executeBasicand(argv) !=-1 && ok==0)
           {
             trouve=1;
           }
...
int main(char** argv,int argc)
{
    ...
    while(1)
    {
       ...
       strcpy(ch1,buf);
            ...
            temp=fopen("/home/chelbi/Projet/Projet-SE/history.txt","a+");
            fprintf(temp,"%s",ch1);
            fclose(temp);
        ...
        {  tokenize_buffer(params1,&nr,buf," ");
            ...
            {
                temp=fopen("/home/chelbi/Projet/Projet-SE/history.txt","r");
                char ch[500];
